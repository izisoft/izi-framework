...
class Command extends Component
{
    ...
    /**
     * @var \izi\caching\Dependency the dependency to be associated with the cached query result for this command
     * @see cache()
     */
    ...
    /**
     * @var callable a callable (e.g. anonymous function) that is called when [[\izi\db\Exception]] is thrown
     ...
     * @param \izi\caching\Dependency $dependency the cache dependency associated with the cached query result.
     ...
     * by its PHP type. You may explicitly specify the PDO type by using a [[izi\db\PdoValue]] class: `new PdoValue(value, type)`,
     ...
     * @param array|\izi\db\Query $columns the column data (name => value) to be inserted into the table or instance
     ...
     * @param string $type the column type. [[\izi\db\QueryBuilder::getColumnType()]] will be called
    ...
    public function execute()
    {
        ...
        try {
            $profile and Izi::beginProfile($rawSql, __METHOD__);

            $this->internalExecute($rawSql);
            $n = $this->pdoStatement->rowCount();

            $profile and Izi::endProfile($rawSql, __METHOD__);
    ...
    {
        list($profile, $rawSql) = $this->logQuery('izi\db\Command::query');

        if ($method !== '') {
            $info = $this->db->getQueryCacheInfo($this->queryCacheDuration, $this->queryCacheDependency);
            if (is_array($info)) {
                /* @var $cache \izi\caching\CacheInterface */
                $cache = $info[0];
                $cacheKey = $this->getCacheKey($method, $fetchMode, '');
                $result = $cache->get($cacheKey);
                if (is_array($result) && isset($result[0])) {
                    Izi::debug('Query result served from cache', 'izi\db\Command::query');
                    return $result[0];
                }
            }
        }
        ...
        try {
            $profile and Izi::beginProfile($rawSql, 'izi\db\Command::query');
            ...
            $profile and Izi::endProfile($rawSql, 'izi\db\Command::query');
        } catch (Exception $e) {
            $profile and Izi::endProfile($rawSql, 'izi\db\Command::query');
            throw $e;
        }
        ...
        if (isset($cache, $cacheKey, $info)) {
            ...
            Izi::debug('Saved query result in cache', 'izi\db\Command::query');
        }
     ...
     *
     * ```php
     * function (\izi\db\Exception $e, $attempt)
     * {
