...
use izi\caching\CacheInterface;
 ...
 *
 * ```php
 * $connection = new \izi\db\Connection([
 *     'dsn' => $dsn,
 *     'username' => $username,
 *     'password' => $password,
 * ]);
...
{
    /**
     * @event \izi\base\Event an event that is triggered after a DB connection is established
     */
    const EVENT_AFTER_OPEN = 'afterOpen';
    /**
     * @event \izi\base\Event an event that is triggered right before a top-level transaction is started
     */
    const EVENT_BEGIN_TRANSACTION = 'beginTransaction';
    /**
     * @event \izi\base\Event an event that is triggered right after a top-level transaction is committed
     */
    const EVENT_COMMIT_TRANSACTION = 'commitTransaction';
    /**
     * @event \izi\base\Event an event that is triggered right after a top-level transaction is rolled back
     */
     ...
     * schema class names or configurations. Please refer to [[Izi::createObject()]] for
     * details on how to specify a configuration.
     *
     * This property is mainly used by [[getSchema()]] when fetching the database schema information.
     * You normally do not need to set this property unless you want to use your own
     * [[Schema]] class to support DBMS that is not supported by Izi.
     */
    public $schemaMap = [
        'pgsql' => 'izi\db\pgsql\Schema', // PostgreSQL
        'mysqli' => 'izi\db\mysql\Schema', // MySQL
        'mysql' => 'izi\db\mysql\Schema', // MySQL
        'sqlite' => 'izi\db\sqlite\Schema', // sqlite 3
        'sqlite2' => 'izi\db\sqlite\Schema', // sqlite 2
        'sqlsrv' => 'izi\db\mssql\Schema', // newer MSSQL driver on MS Windows hosts
        'oci' => 'izi\db\oci\Schema', // Oracle driver
        'mssql' => 'izi\db\mssql\Schema', // older MSSQL driver on MS Windows hosts
        'dblib' => 'izi\db\mssql\Schema', // dblib drivers on GNU/Linux (and maybe other OSes) hosts
        'cubrid' => 'izi\db\cubrid\Schema', // CUBRID
    ];
    /**
     * @var string Custom PDO wrapper class. If not set, it will use [[PDO]] or [[\izi\db\mssql\PDO]] when MSSQL is used.
     * @see pdo
     */
    public $pdoClass;
    /**
     ...
     * The keys of the array are PDO driver names while the values are either the corresponding
     * command class names or configurations. Please refer to [[Izi::createObject()]] for
     * details on how to specify a configuration.
     *
     * This property is mainly used by [[createCommand()]] to create new database [[Command]] objects.
     * You normally do not need to set this property unless you want to use your own
     * [[Command]] class or support DBMS that is not supported by Izi.
     * @since 2.0.14
     */
    public $commandMap = [
        ...
        'sqlite' => 'izi\db\sqlite\Command', // sqlite 3
        'sqlite2' => 'izi\db\sqlite\Command', // sqlite 2
        'sqlsrv' => 'izi\db\Command', // newer MSSQL driver on MS Windows hosts
        'oci' => 'izi\db\oci\Command', // Oracle driver
     ...
     * Use 0 to indicate that the cached data will never expire.
     * @param \izi\caching\Dependency $dependency the cache dependency associated with the cached query results.
     ...
     * @param \izi\caching\Dependency $dependency the preferred caching dependency. If null, it will be ignored.
    ...
    public function getQueryCacheInfo($duration, $dependency)
    {
        ...
        if ($duration === 0 || $duration > 0) {
            if (is_string($this->queryCache) && Izi::$app) {
                $cache = Izi::$app->get($this->queryCache, false);
            } else {
    ...
    public function open()
    {
        ...
        try {
            if ($this->enableLogging) {
                Izi::info($token, __METHOD__);
            }

            if ($enableProfiling) {
                Izi::beginProfile($token, __METHOD__);
            }
            ...
            if ($enableProfiling) {
                Izi::endProfile($token, __METHOD__);
            }
    ...
    public function close()
    {
        ...
        if ($this->pdo !== null) {
            Izi::debug('Closing DB connection: ' . $this->dsn, __METHOD__);
            $this->pdo = null;
        }
    ...
    protected function createPdoInstance()
    {
        ...
        if ($pdoClass === null) {
            ...
            if (isset($driver)) {
                if ($driver === 'mssql') {
                    $pdoClass = 'izi\db\mssql\PDO';
                } elseif ($driver === 'dblib') {
                    $pdoClass = 'izi\db\mssql\DBLibPDO';
                } elseif ($driver === 'sqlsrv') {
                    $pdoClass = 'izi\db\mssql\SqlsrvPDO';
                }
            }
        }
        ...
        if (strncmp('sqlite:@', $dsn, 8) === 0) {
            $dsn = 'sqlite:' . Izi::getAlias(substr($dsn, 7));
        }
    ...
    public function createCommand($sql = null, $params = [])
    {
        ...
        $config['sql'] = $sql;
        /** @var Command $command */
        $command = Izi::createObject($config);
     ...
     * from rollback will be caught and just logged with [[\Izi::error()]].
    ...
    public function getSchema()
    {
        ...
        if (isset($this->schemaMap[$driver])) {
            ...
            $this->_schema = Izi::createObject($config);
    ...
    {
        Izi::configure($this->getQueryBuilder(), $value);
    ...
    protected function openFromPoolSequentially(array $pool, array $sharedConfig)
    {
        ...
        $cache = is_string($this->serverStatusCache) ? Izi::$app->get($this->serverStatusCache, false) : $this->serverStatusCache;
        ...
        foreach ($pool as $i => $config) {
            ...
            /* @var $db Connection */
            $db = Izi::createObject($config);

            try {
                $db->open();
                return $db;
            } catch (\Exception $e) {
                Izi::warning("Connection ({$config['dsn']}) failed: " . $e->getMessage(), __METHOD__);
                if ($cache instanceof CacheInterface) {
