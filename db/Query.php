...
class Query extends Component implements QueryInterface, ExpressionInterface
{
    ...
    /**
     * @var \izi\caching\Dependency the dependency to be associated with the cached query result for this query
     * @see cache()
     * @since 2.0.14
     */
...
PATTERN;
     ...
     * $query = (new \izi\db\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subquery = (new \izi\db\Query)->from('user')->where(['active' => true])
     * $query = (new \izi\db\Query)->from(['activeusers' => $subquery]);
     *
     ...
     * Use boolean `true` to indicate that [[Connection::queryCacheDuration]] should be used.
     * Defaults to `true`.
     * @param \izi\caching\Dependency $dependency the cache dependency associated with the cached result.
     * @return $this the Query object itself
     * @since 2.0.14
     */
