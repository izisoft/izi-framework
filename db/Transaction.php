 ...
 * @since 2.0
 */
class Transaction extends \izi\base\BaseObject
{
    /**
    ...
    public function begin($isolationLevel = null)
    {
        ...
        if ($this->_level === 0) {
            ...
            Izi::debug('Begin transaction' . ($isolationLevel ? ' with isolation level ' . $isolationLevel : ''), __METHOD__);
        ...
        if ($schema->supportsSavepoint()) {
            Izi::debug('Set savepoint ' . $this->_level, __METHOD__);
            // make sure the transaction wasn't autocommitted
            if ($this->db->pdo->inTransaction()) {
                $schema->createSavepoint('LEVEL' . $this->_level);
            }
        } else {
            Izi::info('Transaction not started: nested transaction not supported', __METHOD__);
    ...
    public function commit()
    {
        ...
        $this->_level--;
        if ($this->_level === 0) {
            Izi::debug('Commit transaction', __METHOD__);
            // make sure the transaction wasn't autocommitted
            if ($this->db->pdo->inTransaction()) {
                $this->db->pdo->commit();
            }
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Izi::debug('Release savepoint ' . $this->_level, __METHOD__);
            // make sure the transaction wasn't autocommitted
            if ($this->db->pdo->inTransaction()) {
                $schema->releaseSavepoint('LEVEL' . $this->_level);
            }
        } else {
            Izi::info('Transaction not committed: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function rollBack()
    {
        ...
        $this->_level--;
        if ($this->_level === 0) {
            Izi::debug('Roll back transaction', __METHOD__);
            // make sure the transaction wasn't autocommitted
            if ($this->db->pdo->inTransaction()) {
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Izi::debug('Roll back to savepoint ' . $this->_level, __METHOD__);
            // make sure the transaction wasn't autocommitted
            if ($this->db->pdo->inTransaction()) {
                $schema->rollBackSavepoint('LEVEL' . $this->_level);
            }
        } else {
            Izi::info('Transaction not rolled back: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function setIsolationLevel($level)
    {
        ...
        Izi::debug('Setting transaction isolation level to ' . $level, __METHOD__);
