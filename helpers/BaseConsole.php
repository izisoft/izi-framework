...
use Izi;
use izi\console\Markdown as ConsoleMarkdown;
use izi\base\Model;
...
class BaseConsole
{
    ...
    {
        return mb_strwidth(static::stripAnsiFormat($string), Izi::$app->charset);
    }
    ...
    public static function ansiColorizedSubstr($string, $start, $length)
    {
        ...
        foreach ($textItems as $k => $textItem) {
            ...
            if ($curPos <= $start && $start < $curPos + Console::ansiStrwidth($textItem)) {
                $text = mb_substr($textItem, $start - $curPos, null, Izi::$app->charset);
                $inRange = true;
            } else {
                $text = $textItem;
            }
            ...
            if ($inRange) {
                ...
                if ($diff <= 0) {
                    if ($diff < 0) {
                        $result = mb_substr($result, 0, $diff, Izi::$app->charset);
                    }
            ...
            $curPos += mb_strlen($textItem, Izi::$app->charset);
        }
