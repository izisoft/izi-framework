...
use izi\base\Model;
use izi\db\ActiveRecordInterface;
use izi\validators\StringValidator;
use izi\web\Request;
...
class BaseHtml
{
     ...
     * The [[\izi\base\Application::charset|application charset]] will be used for encoding.
    ...
    {
        return htmlspecialchars($content, ENT_QUOTES | ENT_SUBSTITUTE, Izi::$app ? Izi::$app->charset : 'UTF-8', $doubleEncode);
    }
     ...
     * See [[\izi\web\Request::methodParam]] for more details.
     * @param array $options the tag options in terms of name-value pairs. These will be rendered as
     * the attributes of the resulting tag. The values will be HTML-encoded using [[encode()]].
     * If a value is null, the corresponding attribute will not be rendered.
     * See [[renderTagAttributes()]] for details on how attributes are being rendered.
    ...
    {
        $header = isset($options['header']) ? $options['header'] : '<p>' . Izi::t('izi', 'Please fix the following errors:') . '</p>';
     ...
     * the `maxlength` option will take the max value of [[\izi\validators\StringValidator::max]] and
     * [[\izi\validators\StringValidator::length]].
     * @param Model $model the model object
     ...
     *   by a string validator, the `maxlength` option will take the max value of [[\izi\validators\StringValidator::max]]
     *   and [[\izi\validators\StringValidator::length].
     ...
     * `'data' => ['params' => ['id' => 1, 'name' => 'izi']]` would be rendered as
