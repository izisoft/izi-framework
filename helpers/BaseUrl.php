...
{
    /**
     * @var \izi\web\UrlManager URL manager to use for creating URLs
     * @since 2.0.8
     */
     ...
     *
     * This method will use [[\izi\web\UrlManager]] to create a URL.
     *
     ...
     *
     * - If the route is an empty string, the current [[\izi\web\Controller::route|route]] will be used;
     * - If the route contains no slashes at all (e.g. `index`), it is considered to be an action ID
     *   of the current controller and will be prepended with [[\izi\web\Controller::uniqueId]];
     * - If the route has no leading slash (e.g. `site/index`), it is considered to be a route relative
     *   to the current module and will be prepended with the module's [[\izi\base\Module::uniqueId|uniqueId]].
     *
     ...
     * - `true`: returning an absolute base URL whose scheme is the same as that in [[\izi\web\UrlManager::$hostInfo]].
    ...
    {
        $route = Izi::getAlias((string) $route);
        ...
        // relative route
        if (Izi::$app->controller === null) {
            throw new InvalidArgumentException("Unable to resolve the relative route: $route. No active controller is available.");
        }

        if (strpos($route, '/') === false) {
            // empty or an action ID
            return $route === '' ? Izi::$app->controller->getRoute() : Izi::$app->controller->getUniqueId() . '/' . $route;
        }

        // relative to module
        return ltrim(Izi::$app->controller->module->getUniqueId() . '/' . $route, '/');
    }
     ...
     * [[\izi\web\UrlManager::$hostInfo]]) will be returned. If `$url` is already an absolute URL, its scheme
    ...
    public static function to($url = '', $scheme = false)
    {
        ...
        $url = Izi::getAlias($url);
        if ($url === '') {
            $url = Izi::$app->getRequest()->getUrl();
        }
     ...
     * later by [[previous()]]. If not set, [[\izi\web\User::setReturnUrl()]] will be used with passed URL.
     * @see previous()
     * @see \izi\web\User::setReturnUrl()
     */
    public static function remember($url = '', $name = null)
    {
        ...
        if ($name === null) {
            Izi::$app->getUser()->setReturnUrl($url);
        } else {
            Izi::$app->getSession()->set($name, $url);
        }
    }
     ...
     * If not set, [[\izi\web\User::getReturnUrl()]] will be used to obtain remembered URL.
     ...
     * @see remember()
     * @see \izi\web\User::getReturnUrl()
     */
    public static function previous($name = null)
    {
        if ($name === null) {
            return Izi::$app->getUser()->getReturnUrl();
        }

        return Izi::$app->getSession()->get($name);
    }

    /**
     * Returns the canonical URL of the currently requested page.
     *
     * The canonical URL is constructed using the current controller's [[\izi\web\Controller::route]] and
     * [[\izi\web\Controller::actionParams]]. You may use the following code in the layout view to add a link tag
     * about canonical URL:
     *
     * ```php
    ...
    {
        $params = Izi::$app->controller->actionParams;
        $params[0] = Izi::$app->controller->getRoute();
    ...
    {
        $currentParams = Izi::$app->getRequest()->getQueryParams();
        $currentParams[0] = '/' . Izi::$app->controller->getRoute();
        $route = array_replace_recursive($currentParams, $params);
        return static::toRoute($route, $scheme);
    }

    /**
     * @return \izi\web\UrlManager URL manager used to create URLs
     * @since 2.0.8
     */
    protected static function getUrlManager()
    {
        return static::$urlManager ?: Izi::$app->getUrlManager();
    }
}
